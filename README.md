...
Phân tích dữ liệu doanh số theo thời gian, sản phẩm, khách hàng, kênh bán hàng và khu vực tại Hoa Kỳ, kết hợp Python EDA và Power BI Dashboard để chuyển dữ liệu giao dịch thành các insight hỗ trợ quyết định kinh doanh.

📌 1. Tổng quan dự án

Dự án phân tích dữ liệu doanh số của Acme Co. tại Hoa Kỳ, tập trung vào giai đoạn 2014–2018.

Mục tiêu chính là đánh giá các yếu tố thúc đẩy doanh thu và lợi nhuận, xác định những khu vực/sản phẩm/kênh/khách hàng có hiệu suất cao hoặc thấp, đồng thời phát hiện xu hướng theo thời gian, tính mùa vụ và các giao dịch bất thường.

Dự án được triển khai theo quy trình:
...
Dataset gốc, gồm các bảng Sales Orders, Customers, Products, Regions, State Regions và 2017 Budgets

Regional Sales Analysis.ipynb

Notebook Python thực hiện Data Profiling, Cleaning, Feature Engineering và EDA

Sales_data(EDA Exported).csv

Dataset sau khi xử lý và chuẩn hóa, dùng cho phân tích/dashboard

sales_dashboard.pbix

Power BI Dashboard trực quan hóa các KPI và business insights
...
Sales Orders → State Regions

Sales Orders → 2017 Budgets
...
Cột budget có giá trị thiếu đối với phần lớn dữ liệu vì ngân sách được sử dụng cho năm 2017. Các cột giao dịch chính sau EDA export không có missing values.
...
Các biến này hỗ trợ phân tích xu hướng, mùa vụ và hiệu quả kinh doanh theo thời gian.
...
Phân tích phân phối Average Order Value (AOV) nhằm đánh giá giá trị giao dịch và phát hiện các đơn hàng có giá trị bất thường.
...
Bubble/Scatter Plot được sử dụng để hỗ trợ phân khúc khách hàng.
...
Revenue ↔ Profit: tương quan mạnh, khoảng 0.87

Unit Price ↔ Revenue: khoảng 0.91

Unit Price ↔ Profit: khoảng 0.79

Unit Price ↔ Cost: khoảng 0.94

Cost ↔ Revenue: khoảng 0.85
...
Doanh thu nhìn chung duy trì tương đối ổn định trong giai đoạn phân tích, với một số biến động theo tháng. Dữ liệu cho thấy các giai đoạn tăng doanh thu thường tập trung vào khoảng tháng 5–6, trong khi đầu năm có xu hướng yếu hơn.

Một mức sụt giảm đáng chú ý xuất hiện vào đầu năm 2017, cần được điều tra thêm để xác định nguyên nhân.
...
California là bang nổi bật nhất với khoảng 230M USD doanh thu và hơn 7,500 đơn hàng.

Các thị trường tiếp theo gồm Illinois, Florida và Texas, tạo thành nhóm có hiệu suất tương đối mạnh.

👥 Customer Performance

Nhóm khách hàng dẫn đầu tạo ra doanh thu khoảng 10–12M USD/khách hàng, trong khi nhóm cuối chỉ khoảng 4–5M USD/khách hàng.

Điều này cho thấy doanh thu có sự tập trung đáng kể ở nhóm khách hàng có giá trị cao.

💰 Profitability

Tỷ suất lợi nhuận trong dataset dao động đáng kể giữa các giao dịch. Theo phân tích kênh, mức Average Profit Margin giữa Export, Distributor và Wholesale khá sát nhau:
...
Dashboard hướng tới việc theo dõi:
...
Dashboard giúp chuyển kết quả EDA thành một công cụ trực quan để người dùng có thể filter, drill-down và so sánh hiệu suất kinh doanh.
...
Sau đó chạy các cell theo thứ tự từ:

Data Ingestion → Data Profiling → Data Cleaning → Feature Engineering → EDA
...
Dựa trên kết quả EDA, một số hướng hành động được đề xuất:

Tăng trưởng tại các thị trường yếu
Ưu tiên các khu vực/bang có doanh thu thấp thông qua promotion và chiến lược marketing theo địa phương.

Tập trung vào sản phẩm chủ lực
Duy trì availability và inventory cho nhóm sản phẩm có doanh thu cao như Product 25 và Product 26.

Cải thiện nhóm sản phẩm trung bình/thấp
Thử nghiệm pricing, promotion và cost optimization để cải thiện doanh thu và profitability.

Mở rộng Export Channel
...
Tập trung giữ chân nhóm khách hàng doanh thu cao và triển khai upsell/cross-sell cho nhóm khách hàng có giá trị thấp hơn.

Theo dõi bất thường theo thời gian
Điều tra các giai đoạn doanh thu giảm mạnh, đặc biệt là đầu năm 2017.

Outlier Management
Khi tính các chỉ số trung bình, nên xem xét riêng các giao dịch có quantity/revenue/unit price bất thường để tránh làm sai lệch kết quả.
...
Data Understanding → Data Cleaning → Data Transformation → Exploratory Analysis → Visualization → Business Insights → Dashboard

Các kỹ năng chính được thể hiện:
...
Việc kết hợp Python EDA với Power BI Dashboard giúp chuyển dữ liệu giao dịch thành các insight có khả năng hỗ trợ:
...
Final Output: A data-driven view of sales performance, profitability, customers, channels and regional opportunities.
